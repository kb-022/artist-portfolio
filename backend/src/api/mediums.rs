...
pub async fn update_medium(State(state): State<Arc<AppState>>, Path(slug): Path<String>, Json(body): Json<QueryMedium>) -> Result<(StatusCode, Json<Medium>), (StatusCode, Json<serde_json::Value>)>{
    ...
    if existing_medium.is_some(){
        ...
        sqlx::query_as!(Medium, "UPDATE mediums SET name=$1, slug=$2 WHERE slug=$3 RETURNING *", body.name, new_slug, slug)
            ...
            .map_err(|_| internal_server_error("Failed to update medium"))
    } else {
       Err(not_found_error("medium does not exist"))
...
pub async fn delete_medium(State(state): State<Arc<AppState>>, Path(slug): Path<String>) -> Result<(StatusCode, Json<Medium>), (StatusCode, Json<serde_json::Value>)>{
    ...
    if existing_medium.is_some(){
        ...
        if works_count.count.unwrap() > 0 {
            return Err(conflict_error(&format!("Delete {} works attached to medium first",works_count.count.unwrap())));
        }
